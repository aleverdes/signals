...
{
    public abstract class Signal<T1, T2, T3> : ISignal
    {
        private readonly HashSet<Action> _listenersWithoutParameters = new HashSet<Action>();
        private readonly HashSet<Action<T1, T2, T3>> _listeners = new HashSet<Action<T1, T2, T3>>();
        ...
        {
            var listeners = new HashSet<Action<T1, T2, T3>>(_listeners);
